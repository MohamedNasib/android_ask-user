package com.wesal.domain.gateways.preferencesGateways
...
object PreCon {
    ...
    internal const val LOCATION_LAT = "qqqqq1"
    internal const val LOCATION_LNG = "qqqqq2"
    internal const val NOTIFICATION_OPEN = "qqqqq3"
}

class PrefMangerImplementer(
    private val pref: PreferencesHelper
) : PrefManger {
//    override fun saveUserData(mode: UserModel?) {
//        mode?.let {
//            val toJson = Gson().toJson(mode)
...
//    }
    ...
    override fun isNotificationOpened(): Boolean {
        return pref.load(PreCon.NOTIFICATION_OPEN, true)
    }

    override fun changeNotificationOpen(isOpen: Boolean) {
        pref.save(PreCon.NOTIFICATION_OPEN, isOpen)
    }
...
//        return """Bearer 1Cfn56GLa4AXDKh9"""
    }
...
//        return pref.load(PreCon.LAST_DATABASE_UPDATE, "2020-03-16")
    ...
    override fun saveUserLocation(lat: Double, lng: Double) {
        pref.save(PreCon.LOCATION_LAT, lat.toString())
        pref.save(PreCon.LOCATION_LNG, lng.toString())
    }

    override fun getUserLocation(): Location {
        val lat = pref.load(PreCon.LOCATION_LAT, "0").toDouble()
        val lng = pref.load(PreCon.LOCATION_LNG, "0").toDouble()
    ...
    override fun setUserData(value: UserModel?) {
        value?.let {
            ...
            pref.save(PreCon.USER_MODEL, toJson)
            saveApiToken(it.advertiserApiToken)
            setUserLooged(true)
            setInVisitorMode(false)
        } ?: kotlin.run {
            pref.save(PreCon.USER_MODEL, "")
...
//    fun loadUserData(): UserModel?
    ...
    fun isNotificationOpened(): Boolean
    fun changeNotificationOpen(isOpen:Boolean)
}
